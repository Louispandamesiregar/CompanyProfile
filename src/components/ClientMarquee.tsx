...
export function ClientMarquee() {
  ...
  return (
    <section className="pt-8 md:pt-10 pb-10 bg-slate-950 overflow-hidden border-y border-slate-800">
      <div className="container mx-auto px-4 text-center mb-10">
        <h3 className="text-sm md:text-base font-bold tracking-widest uppercase text-slate-400 mb-2">
          {clients.subtitle}
        </h3>
        ...
        <div className="flex w-max animate-marquee hover:[animation-play-state:paused] items-center">
          {scrollItems.map((client, idx) => (
            ...
            >
              ...
              <span className="mt-4 text-xs md:text-sm font-bold text-white/90 opacity-0 group-hover:opacity-100 transition-opacity duration-300 translate-y-2 group-hover:translate-y-0 text-center max-w-[120px] leading-tight">
                {client.name}
              </span>
            </div>
          ))}
